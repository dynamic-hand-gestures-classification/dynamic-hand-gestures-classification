...
if show_only_scatter_points:
	...
	if show_nodes:
		...
		view.add(scatter)
	#if show_node_labels:
	#	labels_scatter = vs_visuals.Text()
	#	view.add(labels_scatter)
...
def make_skeleton(fn='DataFile1.txt', dataset_path='dataset', debug=False):
	...
	if not Path(save_fn).exists():
		...
		#skeleton.iloc[:, 88:90] = 0.
		#skeleton.iloc[:, 85:87] = skeleton.iloc[:, 85:87] + skeleton.iloc[:, 88:90] / 2
...
def do_noise(sk_row):
		...
		if args.add_noise or True:
			#noise_array    = pos[0].reshape(1,3)
			#noisecol_array = np.array([0.9, 0.2, 0.1, 1.]).reshape(1,4)
...
def draw_one_frame(sk_row, idx, pos_array, col_array, tips_array, tipscol_array, reset_history, debug=False):
	...
	if show_only_scatter_points:
		...
		if show_nodes:
			scatter.set_data(pos_array, edge_color=None, face_color=(0.4, 0.7, 1, 1), size=10)
		#if show_node_labels and False:
		#	#labels_scatter.set_data(pos_array, edge_color=None, face_color=(0.4, 0.7, 1, 1), size=10)
		#	#textviz = scene.Text('<Label>', parent=canvas.scene, color='black', pos=[225, 50, 0], font_size=24.)
		#	for node_num, node_pos in enumerate(pos_array):
		#		nodelabelviz = vs_visuals.Text(str(node_num), parent=canvas.scene, color='white', pos=node_pos, font_size=16.)
		if show_tips_traces:
	...
	try:
		...
		if idx < len(skeleton.index):
			...
			if idx >= len(skeleton.index) and args.exit_on_eof and lastlabel:
				...
				#print(f'Sending ESC event to GUI...')
				#esc_event = Event('esc_event')
				#esc_event.key = 'Escape'
				#on_key_press(event)
				#update(esc_event)
